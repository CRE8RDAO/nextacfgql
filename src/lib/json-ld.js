...
import { campaignPathBySlug } from 'lib/campaigns'; // todo: get this working for campaigns
...
export function CampaignArticleJsonLd({ post = {}, siteTitle = '' }) {
  const { homepage = '', faviconPath = '/favicon.ico' } = config;
  const { title, slug, excerpt, date, author, categories, modified, featuredImage } = campaign;
  const path = campaignPathBySlug(slug);
  const datePublished = !!date && new Date(date);
  const dateModified = !!modified && new Date(modified);
